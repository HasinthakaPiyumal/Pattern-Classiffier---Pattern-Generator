...
def simulate_ecommerce_data(n_samples=1000):
    np.random.seed(43)
    data = {
        'Age': np.random.randint(18, 70, n_samples),
        'AvgMonthlySpend': np.random.normal(50, 30, n_samples).clip(10, 200),
        'NumPurchasesLastYear': np.random.randint(0, 20, n_samples),
        'DaysSinceLastPurchase': np.random.randint(1, 365, n_samples),
        'SupportTicketsLastMonth': np.random.randint(0, 5, n_samples),
        'WebsiteVisitsLastMonth': np.random.randint(0, 50, n_samples),
        'SubscriptionType': np.random.choice(['Basic', 'Premium', 'VIP'], n_samples),
        'ProductCategoryPreference': np.random.choice(['Electronics', 'Clothing', 'Books', 'Home'], n_samples)
    }
    df = pd.DataFrame(data)

    df['Churn'] = 0
    df.loc[(df['DaysSinceLastPurchase'] > 180) & (df['AvgMonthlySpend'] < 30), 'Churn'] = 1
    df.loc[(df['SupportTicketsLastMonth'] >= 3) & (df['SubscriptionType'] == 'Basic'), 'Churn'] = 1
    df.loc[(df['NumPurchasesLastYear'] < 2) & (df['WebsiteVisitsLastMonth'] < 5), 'Churn'] = 1
    df.loc[(df['Churn'] == 0) & (np.random.rand(n_samples) < 0.08), 'Churn'] = 1 
    df.loc[(df['Churn'] == 1) & (np.random.rand(n_samples) < 0.15), 'Churn'] = 0 

    df = pd.get_dummies(df, columns=['SubscriptionType', 'ProductCategoryPreference'], drop_first=True)
    return df.drop('Churn', axis=1), df['Churn']

X, y = simulate_ecommerce_data()
...
class ECommerceXAI:
    def __init__(self, black_box_model, X_train, feature_names):
        ...
        self.class_names = ['No Churn', 'Churn']
    ...
    def global_surrogate_model(self):
        ...
        surrogate_model = LogisticRegression(max_iter=1000, random_state=43)
        surrogate_model.fit(self.X_train, self.black_box_model.predict(self.X_train))
        print("Trained a Logistic Regression surrogate model.")
        ...
        print("Coefficients can be inspected for global feature importance.")
        return surrogate_model

    def explain_local_prediction(self, instance_idx, X_data):
        print(f"\n--- 2. Local Prediction Explanation for customer {instance_idx} ---")
        ...
        if significant_features:
            print(f"Local rule for {self.class_names[original_prediction]} for this customer:")
            rule_parts = []
    ...
    def generate_counterfactual(self, instance_idx, X_data, desired_prediction=0):
        print(f"\n--- 3. Actionable Insights: Counterfactual Explanation for customer {instance_idx} ---")
        ...
        print(f"Attempting to find minimal changes to change prediction to {self.class_names[desired_prediction]} (e.g., prevent churn)...")
        ...
        for _ in range(max_iterations):
            ...
            for i, feature in enumerate(self.feature_names):
                ...
                if X_data[feature].dtype in ['int64', 'float64']:
                    if feature in ['AvgMonthlySpend', 'NumPurchasesLastYear', 'WebsiteVisitsLastMonth']:
                    ...
                    elif feature in ['DaysSinceLastPurchase', 'SupportTicketsLastMonth']:
        ...
        if found_cf:
            ...
            print(f"\nSuggested Actions to Prevent Churn ({self.class_names[desired_prediction]}):")
            changes = {}
    ...
    def subgroup_divergence_analysis(self, X_data, y_true):
        ...
        subgroups = {
            "LowSpend_HighDaysSincePurchase": (X_data['AvgMonthlySpend'] < 20) & (X_data['DaysSinceLastPurchase'] > 200),
            "NewCustomer_LowEngagement": (X_data['NumPurchasesLastYear'] < 1) & (X_data['WebsiteVisitsLastMonth'] < 2),
            "PremiumSub_HighTickets": (X_data['SubscriptionType_Premium'] == 1) & (X_data['SupportTicketsLastMonth'] >= 2)
        }

        print("\nAnalyzing churn prediction performance across defined subgroups:")
    ...
    def interactive_exploration_menu(self, X_data, y_true):
        ...
        print("Welcome to the E-commerce Customer Churn XAI Dashboard!")
        while True:
            print("\nChoose an action:")
            print("1. Get Global Model Understanding")
            print("2. Explain a Specific Customer's Churn Prediction")
            print("3. Find Counterfactual to Prevent Churn")
            ...
            if choice == '1':
                ...
                customer_idx_str = input(f"Enter customer index (0 to {len(X_data) - 1}): ")
                try:
                    customer_idx = int(customer_idx_str)
                    if 0 <= customer_idx < len(X_data):
                        self.explain_local_prediction(customer_idx, X_data)
                    else:
                        print("Invalid index.")
                except ValueError:
                    print("Invalid input.")
                ...
                try:
                    customer_idx = int(customer_idx_str)
                    if 0 <= customer_idx < len(X_data):
                        self.generate_counterfactual(customer_idx, X_data, desired_prediction=0)
                    else:
                        print("Invalid index.")
                except ValueError:
                    print("Invalid input.")
...
ecommerce_xai = ECommerceXAI(black_box_model, X_train, feature_names)
ecommerce_xai.interactive_exploration_menu(X_test, y_test)