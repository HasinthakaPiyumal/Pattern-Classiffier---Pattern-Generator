...
def program2_healthcare():
    ...
    feature_names = ['Age', 'BloodPressure', 'Cholesterol', 'Glucose', 'BMI', 'SymptomsPresent']
    class_names = ['NoDisease', 'Disease']
    
    X[:, 0] = np.random.randint(20, 90, n_samples)
    X[:, 1] = np.random.randint(90, 180, n_samples)
    X[:, 2] = np.random.randint(120, 250, n_samples)
    X[:, 3] = np.random.randint(70, 200, n_samples)
    X[:, 4] = np.random.rand(n_samples) * 20 + 18
    X[:, 5] = np.random.randint(0, 5, n_samples)

    y = np.where((X[:, 0] > 60) & (X[:, 1] > 140) & (X[:, 2] > 200), 1, y)
    y = np.where((X[:, 0] < 40) & (X[:, 4] < 25), 0, y)
    y = np.clip(y, 0, 1)

    X_df = pd.DataFrame(X, columns=feature_names)
    X_train, X_test, y_train, y_test = train_test_split(X_df, y, test_size=0.2, random_state=42)

    bb_model = GradientBoostingClassifier(n_estimators=100, random_state=42)
    bb_model.fit(X_train, y_train)
    
    output = []

    surrogate_model = LogisticRegression(random_state=42, solver='liblinear')
    surrogate_model.fit(X_train, bb_model.predict(X_train))
    output.append(f"GLOBAL UNDERSTANDING (Logistic Regression Surrogate):\nSurrogate Model Accuracy: {accuracy_score(y_test, surrogate_model.predict(X_test)):.2f}\nCoefficients (Feature Importance):\n" +
                  "\n".join([f"  {feature_names[i]}: {surrogate_model.coef_[0][i]:.2f}" for i in range(len(feature_names))]))
    ...
    output.append(f"\nExplaining Patient Instance {instance_idx}: True Label={class_names[true_label]}, BB Prediction={class_names[bb_prediction]} (Proba: {bb_proba[0]:.2f}/{bb_proba[1]:.2f})")
    ...
    subgroup_mask = (X_test['Age'] > 65) & (X_test['BMI'] > 30)
    ...
    if len(subgroup_y) > 0:
        ...
        output.append(f"Mean Absolute SHAP for 'Disease' in Subgroup:\n" +
                      "\n".join([f"  {feature_names[i]}: {mean_shap_subgroup[i]:.2f}" for i in range(len(feature_names))]))

    output.append(f"\nINTERACTIVE EXPLORATION (SIMULATION):\nThis interface allows drilling down into patient profiles. For example, comparing the current patient's explanation with another:\nPatient {instance_idx} (predicted {class_names[bb_prediction]}) shows high {exp_lime.as_list()[0][0]} importance. Another patient might have higher {feature_names[np.random.randint(0, len(feature_names))]} importance, leading to different treatment considerations.")
    
    return "\n".join(output)

print(program2_healthcare())