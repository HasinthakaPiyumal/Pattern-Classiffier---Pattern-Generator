...
def program3_ecommerce():
    ...
    feature_names = ['UserAge', 'PurchaseHistoryValue', 'BrowsingFrequency', 'CategoryAffinity', 'TimeSpentOnSite']
    class_names = ['NotPurchased', 'Purchased']
    
    X[:, 0] = np.random.randint(18, 70, n_samples)
    X[:, 1] = np.random.randint(0, 1000, n_samples)
    X[:, 2] = np.random.randint(1, 50, n_samples)
    X[:, 3] = np.random.rand(n_samples)
    X[:, 4] = np.random.randint(10, 500, n_samples)

    y = np.where((X[:, 1] > 500) & (X[:, 2] > 20) & (X[:, 3] > 0.7), 1, y)
    y = np.where((X[:, 1] < 100) & (X[:, 0] < 25), 0, y)
    y = np.clip(y, 0, 1)
    ...
    output.append(f"\nExplaining User Instance {instance_idx}: True Label={class_names[true_label]}, BB Recommendation={class_names[bb_prediction]} (Proba: {bb_proba[0]:.2f}/{bb_proba[1]:.2f})")
    ...
    output.append(f"\nLOCAL EXPLANATION (LIME):\nFeatures contributing to recommendation {class_names[bb_prediction]}:\n" +
    ...
    output.append(f"\nLOCAL EXPLANATION (SHAP) for recommendation {class_names[bb_prediction]}:\n" +
    ...
    subgroup_mask = (X_test['PurchaseHistoryValue'] < 100) & (X_test['BrowsingFrequency'] < 5)
    ...
    if len(subgroup_y) > 0:
        ...
        output.append(f"Mean Absolute SHAP for 'Purchased' in Subgroup:\n" +
                      "\n".join([f"  {feature_names[i]}: {mean_shap_subgroup[i]:.2f}" for i in range(len(feature_names))]))

    output.append(f"\nINTERACTIVE EXPLORATION (SIMULATION):\nThe platform allows product managers to understand why certain recommendations are made. For User {instance_idx}, {exp_lime.as_list()[0][0]} was key. A 'what-if' scenario could explore how increased {feature_names[np.random.randint(0, len(feature_names))]} might alter recommendations, or analyze behavior of users in the 'New Users' segment.")
    
    return "\n".join(output)

print(program3_ecommerce())